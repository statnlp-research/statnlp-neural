...
class BatchTensorNetwork:
    ...
    def __init__(self, fm, batch_network_id, batch_networks,
                 batch_network_id_range):  # network_ids, instances, fm, num_stage = -1, num_row = -1, num_hyperedge = -1, staged_nodes = None, batch_size
        ...
        self.max_num_row = np.amax(np.concatenate([np.expand_dims(
            np.concatenate((network.num_row, np.full((self.max_num_stage - network.num_stage), 0, dtype=np.int)),
                           axis=0), axis=0) for network in batch_networks], axis=1),
                                   axis=0)  # num_stage      the max number of nodes at each stage in the batch
        self.max_num_hyperedge = np.amax(np.concatenate([np.expand_dims(
            np.concatenate((network.num_hyperedge, np.full((self.max_num_stage - network.num_stage), 0, dtype=np.int)),
                           axis=0), axis=0) for network in batch_networks], axis=1),
                                         axis=0)  # num_stage      the max number of hyperedges at each stage in the batch
        ...
        for i in range(self.max_num_stage):
            self.children[i] = np.full(
                (self.batch_size, max_max_num_row, max_max_num_hyperedge, NetworkConfig.HYPEREDGE_ORDER), -1,
                dtype=np.int64)
                ...
                for row in range(network.num_row[stage_idx]):
                    for hyperedge in range(network.num_hyperedge[stage_idx]):
                        for ORDER in range(NetworkConfig.HYPEREDGE_ORDER):
                            self.children[stage_idx][batch_id, row, hyperedge, ORDER] = all_children_list[
                                                                                            row, hyperedge, ORDER] + batch_id * offset
    ...
    def inside(self):
        ...
        for stage_idx in range(1, self.max_num_stage):  ## starting from stage Idx = 1
            ...
            for_expr = torch.sum(torch.take(self.inside_scores, childrens_stage),
                                 3)  # batch_size * num_node * num_hyperedge
            ...
            emission_expr = torch.take(self.nn_batch_output, self.stagenodes2nodeid2nn[stage_idx]).view(self.batch_size,
                                                                                                        self.max_max_num_row,
                                                                                                        1).expand(
                self.batch_size, self.max_max_num_row, self.max_max_num_hyperedge)
            ...
            if not NetworkConfig.IGNORE_TRANSITION:
                trans_expr = torch.take(self.gnp.transition_mat, self.trans_id[
                    stage_idx])  # batch_size * num_nodes  in this stage * num_hyperedge
        ...
        if NetworkConfig.NEUTRAL_BUILDER_ENABLE_NODE_TO_NN_OUTPUT_MAPPING:
            ...
            if is_train:
                ...
                if self.fm.gnp.network2stagenodes2nodeid2nn[self.batch_network_id] == None:
                    nodeid2nn = torch.LongTensor(self.fm.build_node2nn_output_batch(self)).to(
                        NetworkConfig.DEVICE)  # batch_size * num_nodes
                    ...
                    for stage_idx in range(self.max_num_stage):
                        ...
                        self.stagenodes2nodeid2nn[stage_idx] = torch.gather(nodeid2nn, 1, self.staged_nodes[
                            stage_idx])  # nodeid2nn[self.staged_nodes[stage_idx]]
    ...
    def touch_stage(self, stage_idx):
        ...
        if stage_idx > 0:
            if not NetworkConfig.IGNORE_TRANSITION:
                children_list_k_batch = self.get_children(
                    stage_idx)  # self.batch_size, max_max_num_row, max_max_num_hyperedge, NetworkConfig.HYPEREDGE_ORDER
                ...
                for nid in range(self.batch_size):
                    ...
                    for idx in range(len(children_list_k_batch[nid])):
                        ...
                        for children_k_index in range(len(children_list_k_batch[nid][idx])):
                            ...
                            rhs = [self.get_label_id(nid, child_k) for child_k in children_k if
                                   child_k < self.sizes[nid]]
                            if len(rhs) > 0:
    ...
    def max(self):
        ...
        for stage_idx in range(1, self.max_num_stage):  ## starting from stage Idx = 1
            ...
            max_id_list = max_id_list.view(self.batch_size, self.max_max_num_row, 1, 1).expand(self.batch_size,
                                                                                               self.max_max_num_row, 1,

...
class NetworkModel(nn.Module):
    ...
    def learn_batch(self, train_insts, max_iterations, dev_insts, test_insts, optimizer='adam', batch_size=10):
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            batch_tensor_label_networks = BatchTensorNetwork(self.fm, batch_idx * 2 + 0, batch_label_networks,
                                                             batch_network_id_range)
            batch_tensor_unlabel_networks = BatchTensorNetwork(self.fm, batch_idx * 2 + 1, batch_unlabel_networks,
                                                               batch_network_id_range)
        ...
        for iteration in range(max_iterations):  # Epoch
            ...
            end_time = time.time()
            print(colored("Epoch ", 'yellow'), iteration, ": Obj=", all_loss, '\tTime=', end_time - start_time,
                  flush=True)
    ...
    def learn(self, train_insts, max_iterations, dev_insts, test_insts, optimizer_str='adam', batch_size=1):
        ...
        for iteration in range(max_iterations):
            ...
            end_time = time.time()
            print(colored("Iteration ", 'yellow'), iteration, ": Obj=", all_loss,
                  '\tTime={:.2f}s'.format(end_time - start_time), flush=True)
            print()
    ...
    def learn_lbfgs(self, train_insts, max_iterations, dev_insts):
        ...
        def closure():
            ...
            print(colored("Iteration ", 'yellow'), self.iteration, ": Obj=", all_loss.item(), '\tTime=',
            ...
            end_time = time.time()
            print("Prec.: {0:.2f} Rec.: {1:.2f} F1.: {2:.2f}".format(ret[0], ret[1], ret[2]),
                  '\tTime={:.2f}s'.format(end_time - start_time), flush=True)
    ...
    def decode_batch(self, instances, batch_size=10):
        ...
        for batch_idx, batch in enumerate(batches):
            ...
            batch_tensor_unlabel_networks = BatchTensorNetwork(self.fm, batch_idx, batch_unlabel_networks,
                                                               batch_network_id_range)

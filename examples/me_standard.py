...
class LRNeuralBuilder(NeuralBuilder):
    def __init__(self, gnp, voc_size, label_size, dropout=0.5, model="cnn"):
        ...
        self.windows = [3, 4, 5]
        if model == "cnn":
            self.nn_model = nn.ModuleList(
                [nn.Conv2d(self.input_channel, self.num_filters, (K, self.token_embed)).to(NetworkConfig.DEVICE) for K
                 in self.windows])
            """K: 3,4,5
            """
        else:
            self.nn_model = nn.LSTM(self.token_embed, self.num_filters, batch_first=True, bidirectional=True).to(
    ...
    def build_nn_graph(self, instance):
        ...
        if self.model == "lstm":
            ...
            x = [F.relu(conv(word_rep)).squeeze(3) for conv in
                 self.nn_model]  # [(1, hidden_size, sent_len -2), ...]*len(Ks)
